...
struct route_cost {
  ...
  route_cost(route_cost const& o) : route(o.route), cost(o.cost) { }
  void operator=(route_cost const& o) {
    route = o.route;
    cost = o.cost;
  }
...
route_cost find_best_route(int const* distances, int N) {
  ...
  #pragma acc enter data copyin(best_thread[0:threads]) copyin(distances[0:N*N])
  #pragma acc parallel num_gangs(num_blocks) vector_length(block_size) \
  ...
  for (long i = 0; i < threads; ++i) {
    best_route = route_cost::minf(best_route, best_thread[i]);
  }
